    ...
    try {
      ...
      if (shouldReplaceToken) {
        ...
        if (tokenDataStr) {
          ...
          if (tokenResponse.ok) {
            ...
            console.error(
              '[TOKEN] Failed to fetch token (KV empty + HTTP failed)',
            )
          }
        }
        ...
        if (realToken) {
          ...
          if (authHeader?.includes('fake-token')) {
            newHeaders.set(
              'Authorization',
              authHeader.replace('fake-token', realToken),
            )
          }
      ...
      if (url.pathname === '/card/getGameDetail') {
        ...
        try {
          ...
          console.error(
            '[CHINESE_SERVER] Failed to parse JSON response:',
            error,
          )
          return new Response(
            JSON.stringify({
      ...
      if (url.pathname === '/search/getGameList' && request.method === 'POST') {
        ...
        if (responseData.data?.list) {
          ...
          console.log(
            `[SEARCH] Filtered games: ${originalCount} -> ${filteredCount} (Steam only)`,
          )
      ...
      if (
        url.pathname === '/card/getNewsGuideDetail' &&
        request.method === 'POST'
      ) {
      ...
      if (
        url.pathname === '/card/getIndexList' &&
        (request.method === 'POST' || request.method === 'GET')
      ) {
        console.log(
          `[FREE_GAMES] ${request.method} request - Reading free games from KV storage`,
        )
        ...
        try {
          ...
          if (request.method === 'GET') {
            ...
            console.log(
              `[FREE_GAMES] GET request with topic_type=${topicTypeParam} (returning all topics)`,
            )
          }
          // OLD v3: POST request (no topic_type filtering)
          else if (request.method === 'POST') {
            console.log(
              '[FREE_GAMES] POST request (backward compatibility mode)',
            )
          }
      ...
      if (url.pathname === '/base/getBaseInfo' && request.method === 'POST') {
        ...
        if (!response.ok) {
          return new Response(
            JSON.stringify({ code: 500, msg: 'Failed to fetch base info' }),
            {
              status: 500,
              headers: { 'Content-Type': 'application/json', ...allHeaders },
            },
      ...
      if (
        url.pathname === '/cloud/game/check_user_timer' &&
        request.method === 'POST'
      ) {
        // Reuse bodyText if we already read it during token replacement
        if (!bodyText) {
      ...
      if (url.pathname === '/game/getDnsIpPool' && request.method === 'POST') {
        ...
        if (!dnsPoolResponse.ok) {
          return new Response(
            JSON.stringify({ code: 500, msg: 'Failed to fetch DNS pool' }),
            {
              status: 500,
              headers: { 'Content-Type': 'application/json', ...allHeaders },
            },
      ...
      if (url.pathname === '/game/getSteamHost' && request.method === 'GET') {
        ...
        if (!hostsResponse.ok) {
          return new Response(
            JSON.stringify({ code: 500, msg: 'Failed to fetch Steam hosts' }),
            {
              status: 500,
              headers: { 'Content-Type': 'application/json', ...allHeaders },
            },
      ...
      if (
        ...
        if (!type || !TYPE_TO_MANIFEST[type]) {
          return new Response(
            JSON.stringify({ code: 400, msg: 'Invalid type parameter' }),
            {
              status: 400,
              headers: { 'Content-Type': 'application/json', ...allHeaders },
            },
        ...
        if (!response.ok) {
          return new Response(
            JSON.stringify({ code: 500, msg: 'Failed to fetch manifest' }),
            {
              status: 500,
              headers: { 'Content-Type': 'application/json', ...allHeaders },
            },
